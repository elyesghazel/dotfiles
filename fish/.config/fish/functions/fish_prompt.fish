...
#
# Ported from isaaclins/.config. Replaces starship — see config.fish.
# Note: upstream calls `hostname -s`, which Arch does not ship by default
# (that is inetutils). Uses fish's builtin `prompt_hostname` instead.
#
# Tokyo Night palette:
